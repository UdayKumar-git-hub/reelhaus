...
import { Film, Sparkles, Instagram, Target, Scissors, BookOpen, Clock, Star, ArrowRight } from 'lucide-react';
...
const App = () => {
  ...
  const handleMouseMove = (event) => {
    ...
    const rotateXValue = -1 * ((y - height / 2) / (height / 2)) * 12;
  ...
  // For the grand juniors welcome animation
  const welcomeRef = React.useRef(null);
  const { scrollYProgress: welcomeScrollYProgress } = useScroll({
    target: welcomeRef,
  ...
  return (
    <div className="min-h-screen bg-black text-gray-200 font-sans overflow-x-hidden">
      ...
      <div className="relative z-10 bg-black">
        ...
        <section ref={welcomeRef} className="py-24 flex items-center justify-center">
            <div className="max-w-4xl mx-auto px-4 text-center">
                ...
                >
                    Welcome, creators! This isn't just a club; it's your launchpad. A place to experiment, to create bold content, and to shape the digital voice of IARE. We didn't just save you a seat—<span className="font-bold text-yellow-400">we built this stage for you.</span> Ready to make your mark?
                </motion.p>
            </div>
        </section>
        ...
        <section className="pt-0 pb-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style={{ perspective: '1200px' }}>
            <motion.div
              ...
              transition={{ duration: 0.8 }}
            >
              <h2 className="text-lg font-semibold text-yellow-400 uppercase tracking-widest animate-pulse">Our Inaugural Event</h2>
              <p className="mt-4 text-4xl md:text-6xl font-black text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.2)]">The Stage is Set.</p>
            </motion.div>

            <motion.div
              className="relative group rounded-3xl overflow-hidden shadow-2xl shadow-yellow-500/10"
            ...
            >
              <div className="absolute inset-0 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl transition-all duration-300 ease-out group-hover:border-yellow-400/50"></div>
              ...
              <div className="relative p-8 sm:p-12">
                <div className="text-center">
                  <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{once: true}} transition={{ delay: 0.2, type: 'spring', stiffness: 150 }}>
        ...
        <section className="py-24 bg-black">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                <motion.div 
                    ...
                    transition={{ duration: 0.8 }}
                >
                    <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Ready to Join The Crew?</h2>
                    <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto">
                        We're looking for passionate creators, storytellers, and visionaries. If you're ready to learn, collaborate, and make an impact, your spot is waiting.
                    </p>
                    <motion.a
                        href="#" // Replace with your hiring/application link
                        className="group relative inline-flex items-center justify-center px-10 py-5 text-xl font-bold text-black bg-yellow-400 rounded-full overflow-hidden transition-all duration-300 transform hover:scale-105 shadow-2xl shadow-yellow-500/30"
                        whileHover={{ y: -5, boxShadow: '0 20px 25px -5px rgba(234,179,8,0.4), 0 10px 10px -5px rgba(234,179,8,0.3)' }}
                        whileTap={{ y: 1 }}
                    >
                        <span className="absolute w-0 h-0 transition-all duration-500 ease-out bg-white/30 rounded-full group-hover:w-56 group-hover:h-56"></span>
                        <span className="relative flex items-center">Apply Now <ArrowRight className="w-6 h-6 ml-3" /></span>
                    </motion.a>
                </motion.div>
            </div>
        </section>
