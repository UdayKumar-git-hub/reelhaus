...
  const y = useTransform(scrollYProgress, [0, 0.2], [0, -100]);
  ...
  const starScale = useTransform(welcomeScrollYProgress, [0.1, 0.5, 0.9], [0, 1.2, 0]);
  const starOpacity = useTransform(welcomeScrollYProgress, [0.1, 0.2, 0.8, 0.9], [0, 1, 1, 0]);
  const text1Y = useTransform(welcomeScrollYProgress, [0.2, 0.4], ['20px', '0px']);
  const text1Opacity = useTransform(welcomeScrollYProgress, [0.2, 0.3, 0.8, 0.9], [0, 1, 1, 0]);
  const text2Y = useTransform(welcomeScrollYProgress, [0.4, 0.6], ['20px', '0px']);
  const text2Opacity = useTransform(welcomeScrollYProgress, [0.4, 0.5, 0.8, 0.9], [0, 1, 1, 0]);
...
        <section ref={welcomeRef} className="h-screen flex items-center justify-center sticky top-0">
            <div className="max-w-4xl mx-auto px-4 text-center">
                <motion.div 
                    className="inline-block p-4 mb-6 bg-yellow-500/10 rounded-full shadow-[0_0_40px_10px_rgba(234,179,8,0.5)]"
                    style={{scale: starScale, opacity: starOpacity}}
                >
                    <Star className="w-12 h-12 text-yellow-400"/>
                </motion.div>
                <motion.h2 style={{y: text1Y, opacity: text1Opacity}} className="text-5xl md:text-6xl font-bold text-white mb-6">To the New Architects of Imagination,</motion.h2>
                <motion.p style={{y: text2Y, opacity: text2Opacity}} className="text-xl md:text-2xl text-gray-300 leading-relaxed">
                    Welcome, Visionaries. You are not just joining a club; you are inheriting a legacy and beginning your own. This is your space to challenge norms, to create without fear, and to tell stories that will define the future of IARE's digital identity. We didn't just save you a seat—<span className="font-bold text-yellow-400">we built this stage for you.</span> Let's create something unforgettable.
                </motion.p>
            </div>
        </section>
